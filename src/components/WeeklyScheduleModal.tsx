            ...
            {isLoading ? (
              ...
              <div className="space-y-6">
                {days.map((dayName) => {
                  ...
                  return (
                    <div key={dayName} className="border border-neutral-300 dark:border-neutral-700 rounded-lg p-4">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center space-x-3">
                          <h4 className="text-md font-medium text-neutral-950 dark:text-neutral-100 capitalize">
                            {dayName}
                          </h4>
                      ...
                      {day.is_active && (
                        <div className="space-y-3">
                          {day.shifts.length === 0 ? (
                            <p className="text-sm text-neutral-600 dark:text-neutral-400 italic">No shifts scheduled</p>
                          ) : (
                            day.shifts.map((shift, shiftIndex) => (
                              <div key={shiftIndex} className="bg-neutral-100 dark:bg-neutral-700 rounded-md p-3">
                                <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
                                  {/* Service */}
                                  <div>
                                    <label className="block text-xs font-medium text-neutral-800 dark:text-neutral-300 mb-1">
                                      Service
                                    </label>
                                    <select
                                      value={extractServiceId(shift.service_id) || ''}
                                      onChange={(e) => handleShiftChange(dayName, shiftIndex, 'service_id', e.target.value)}
                                      className="w-full rounded-md border border-neutral-400 dark:border-neutral-600 px-2 py-1 text-sm focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500 bg-white dark:bg-neutral-800 text-neutral-950 dark:text-neutral-100"
                                    >
